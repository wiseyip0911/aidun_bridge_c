...
设计目标:让 Hermes(或任意本机 Agent)在不写代码的前提下,通过 4 个简单
命令完成「收任务 -> 处理 -> 回投结果」的闭环。

推荐使用方法
=============

安装本仓库后,这个模块会注册为 console script ``aidun-hermes-worker``,
之后在**任意工作目录**下都能直接调用:

    aidun-hermes-worker list                       # 列出当前待处理任务
    aidun-hermes-worker show <rid>                 # 看某条任务的完整 JSON
    aidun-hermes-worker reply <rid> "<回复文本>"    # 把文本结果回投给请求方
    aidun-hermes-worker done <rid>                 # 不回复,只把这条任务从本地丢掉

``rid`` 支持只写前 8 位(或更长前缀),会自动匹配唯一文件。

进阶用法(很少用到):

    aidun-hermes-worker reply-json <rid> '<json>'  # 用任意 JSON 作为 payload 回复
    aidun-hermes-worker list --channel chat        # 只列 channel=chat 的任务
    aidun-hermes-worker list --json                # 以 JSON 数组输出(给程序消费)
    aidun-hermes-worker watch --interval 5         # 前台长跑监控(给运维人查活)

也可以等价地用模块形式 ``python -m aidun_bridge_c.hermes_worker ...``,或者
直接执行仓库根的 ``hermes_worker.py``(向后兼容,内部调用相同 main())。

退出码:0 成功,2 参数/匹配错误,1 内部错误。

为什么独立成 console script
============================

Hermes 的 LocalEnvironment 在 Windows 下解析含反斜杠的绝对路径有 bug
(`D:\\aidun\\aidun_bridge_c\\hermes_worker.py` 会被压成
`D:\\aidunaidun_bridge_chermes_worker.py`),且 cwd 切换也不可靠。
console script 完全不依赖 cwd / 路径分隔符,可以稳定地被 agent 调用。
"""
...
def _candidate_env_dirs() -> list[Path]:
    ...
    1. 当前工作目录及其父目录(对开发期"在仓库根直接 python ..."友好)。
    2. 本文件所在 package 的父目录及其父目录(对 console_script 友好——
       被 agent 在 / 或 ~ 之类目录调用时,仍能定位到 editable install
       的仓库根)。
...
def _autoload_dotenv() -> None:
    """在多个候选目录里找 ``.env`` 并加载到 os.environ。

    用 python-dotenv(bridge-c-core 已经把它列成依赖),退化到简单手解析。
    缺包/缺文件都安静跳过。一旦找到第一份 .env 就停。
    """
    try:
        from dotenv import load_dotenv  # type: ignore

        for parent in _candidate_env_dirs():
...
def _pool_dir() -> Path:
    ...
    优先级:
    1. env ``KQ_POOL_LOCAL_POOL_DIR``(.env 里若改了路径,也跟着走)。
    2. 当前工作目录下的 ``data/pending``(在仓库根跑时的最自然行为)。
    3. 仓库根(本文件 src/aidun_bridge_c/../../data/pending)的 ``data/pending``。

    第 3 条让 agent 在任意 cwd 下都能正确定位到 editable install 仓库
    根的池子,不依赖于命令调用方先 cd。
    """
    env_dir = os.environ.get("KQ_POOL_LOCAL_POOL_DIR", "").strip()
    if env_dir:
        p = Path(env_dir)
        return p if p.is_absolute() else (Path.cwd() / p).resolve()

    cwd_default = Path.cwd() / "data" / "pending"
    if cwd_default.exists():
        return cwd_default

    repo_root = Path(__file__).resolve().parents[2]
    return repo_root / "data" / "pending"
...
def cmd_list(args: argparse.Namespace) -> int:
    ...
    if not records:
        print(f"(待处理任务:0,pool={_pool_dir()})")
        return 0

    print(f"待处理任务:{len(records)}  pool={_pool_dir()}")
    print("-" * 78)
...
def _summarize(rec: dict, fp: Path) -> str:
    ...
    lines.append(
        f"  → 处理后运行: aidun-hermes-worker reply {rid[:8]} \"<回复文本>\""
    )
    return "\n".join(lines)
...
def cmd_watch(args: argparse.Namespace) -> int:
    ...
    if not once and not quiet:
        print(
            f"[watch] 监听 {pool.resolve()}, 每 {interval}s 扫描一次。"
            " 按 Ctrl+C 退出。"
        )
...
def build_parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(
        prog="aidun-hermes-worker",
        description="Aidun bridge_c 本机 Hermes/Agent 任务处理工具。",
    )
